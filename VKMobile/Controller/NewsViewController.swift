...
class NewsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITableViewDataSourcePrefetching, NewsTextCellSizeDelegate {
    ...
    func getAllNews() {
        service.getNewsFeed(startFrom: "") { [weak self] (news, next) in
            newsArray = news
            self?.nextFrom = next
    ...
    func getNewsRefresh() {
        ...
        service.getNewsFeedRefresh(startTime: startTime) { [weak self] (news) in
    ...
    func getNewsMore() {
        isLoading = true
        service.getNewsFeed(startFrom: nextFrom) { [weak self] (news, next) in
