//
//  PhotoCacheServiceAlt.swift
//  VKMobile
//
//  Created by Grigory on 24.01.2021.
//

import Foundation
import Alamofire

protocol PhotoCacheServiceAltDelegate: class {
    func imageLoaded(at indexPath: IndexPath)
}

class PhotoCacheServiceAlt {
    
    weak var delegate: PhotoCacheServiceAltDelegate?
    ...
    private func loadImageFromNet(at indexPath: IndexPath, byUrl url: URL, directory: String) {
        ...
        AF.request(url).responseData(queue: DispatchQueue.global()) { [weak self] response in
            ...
            DispatchQueue.main.async {
                self?.delegate?.imageLoaded(at: indexPath)
            }
        }
    }
